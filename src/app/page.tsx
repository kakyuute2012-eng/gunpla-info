...
// TODO: Supabase接続後はDBから取得する
async function getProducts() {
  // const { data } = await supabase.from("products").select("*, stock_info(*)").order("updated_at", { ascending: false });
    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      <section className="mb-8">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {products
            .filter((p) => p.stock_info.some((s) => s.status === "in_stock"))
            .map((product) => (
