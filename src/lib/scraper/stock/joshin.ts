...
/**
 * ジョーシンウェブショップ 商品ページの在庫状況をチェック
 */
export async function checkJoshinStock(url: string): Promise<StockCheckResult> {
  try {
    ...
    const priceSelectors = [
      ".priceTxt", ".priceArea .price", ".itemPrice", "[class*='price']",
    ];
    ...
    // 予約
    if (bodyText.includes("予約受付") || bodyText.includes("ご予約商品")) {
